...
## Visão Geral
A API MCP Vale Server oferece verificação de qualidade de texto usando Vale com múltiplos conjuntos de regras.
...
#### Exemplo com Texto Problemático
```bash
curl -X POST http://localhost:3000/vale/lint \
  -H "Content-Type: application/json" \
  -d '{
...
```json
{
  "result": {
    "content": [
      {
        "text": "Vale Lint Results for clean.md\nTotal Issues: 0\n\n✅ No issues found! Your text follows all Vale style guidelines.\n\n📊 Analysis Summary:\n- Text analyzed successfully\n- Checked rules: Vale, Google, proselint, alex, WC-Styles, Aspect\n- Text quality: Excellent!"
      }
    ]
  }
}
...
#### Exemplo sem fileName (usa padrão)
```bash
curl -X POST http://localhost:3000/vale/lint \
  -H "Content-Type: application/json" \
  -d '{
...
- **400 Bad Request**: Parâmetros inválidos (ex: texto não fornecido)
...
### Vale não instalado
Se o Vale não estiver instalado, a análise retornará erro 500:
...
1. **Vale**: Regras básicas do Vale
2. **Google**: Google Style Guide
3. **proselint**: Verificações de prosa (clareza, concisão)
4. **alex**: Linguagem inclusiva e não ofensiva
5. **WC-Styles**: Regras personalizadas (máx. 20 palavras por frase)
